...
async function checkAndPost() {
  ...
  const statusChanged = lastKnownOnline !== null && lastKnownOnline !== online;
  ...
  try {
    ...
    if (statusChanged) {
      const alert = online
        ? `🟢 **HRX SMP is now Online!** Come join!\n\`${SERVER_HOST}:${SERVER_PORT}\``
        : `🔴 **HRX SMP just went Offline.**`;
      await channel.send({ content: `@everyone ${alert}` });
    }
